...
# # Uncomment to create files for tissue/pathway enrichment analysis
# dir.create( 'fuma', showWarnings = FALSE )
# dir.create( 'pascal', showWarnings = FALSE )
# walk( 1:4, function( pc_number ){
#         full_category_pcs$pval[ , c( 2, pc_number + 2 ) ] %>%
#             transmute( chr       = str_match( variant, '^[0-9Xx]+'  )[ , 1 ],
#                        position  = str_match( variant, ':([0-9]+):' )[ , 2 ],
#                        'P-value' = full_category_pcs$pval[ , pc_number + 2 ] ) %>%
#             as_tibble %>%
#             write_tsv( sprintf( 'fuma/body_pc%s_both_sexes.tsv',
#                                 pc_number ) )
#         full_category_pcs$pval[ , c( 1, pc_number + 2 ) ] %>%
#             as_tibble %>%
#             write_tsv( sprintf( 'pascal/body_pc%s_both_sexes.tsv',
#                                 pc_number ),
#                        col_names = FALSE )
#     } )
